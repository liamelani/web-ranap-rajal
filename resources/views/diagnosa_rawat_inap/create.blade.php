...
@section('content')
    <div class="container">
        ...
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header">Tambah Data Diagnosa Rawat Inap</div>

                    <div class="card-body">
                        <form method="POST" action="{{ route('diagnosa_rawat_inap.store') }}">
                            @csrf

                            <div class="form-group">
                                <label for="no_diag_inap">No. Diagnosa Inap</label>
                                <input id="no_diag_inap" type="text" class="form-control" name="no_diag_inap" value="{{ old('no_diag_inap') }}" required autofocus>
                            </div>
                            ...
                            <div class="form-group">
                                ...
                                <select id="no_rm_inap" class="form-control" name="no_rm_inap" required>
                                    ...
                                    @foreach($pasienRawatInap as $pasien)
                                        <option value="{{ $pasien->No_RM_Inap }}">{{ $pasien->No_RM_Inap }} - {{ $pasien->Nama_Pasien }}</option>
                                    @endforeach
                                </select>
                                @error('no_rm_inap')
